...
output = open(r'E:\OneDrive\学术\PPI article\data\mapped data\8. Geneset verify\ID\ID_verified1000000.txt','w+')
    ...
    for geneset in verify_geneset:
        ...
        for random_gene in random_geneset:
            ...
            if random_inset >= inset:
                count_times += 1
        output.write(geneset[0]+'\t'+str(count_times)+'\t'+str(inset)+'\t'+str(mapped_gene)+'\t'+str(len(geneset)-1)+'\n')
    output.close()
